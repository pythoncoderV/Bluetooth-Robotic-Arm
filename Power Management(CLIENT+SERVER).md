...
My plan is to power everything through one power source connected to the PCA9685 V+ and V-. The ground and power of the PCA9685 would be connected to the ground
and power of the ESP-WROOM-32 so one power source would power everything. The PCA9685 requires a voltage of 5V-6V so I went to the upper limit at 6V. To find the current needed for the server arm, I need to find the current each component takes and find the sum (not counting stall current). 
...
The client uses an ESP-WROOM-32 microcontroller, a 5 pin 2 axis(x,y) Push Button Joystick, and a 2 pin switch. I will connect a 3.7V 300mA LiPo battery through the 
...
stable signal for the arm's motion. 
